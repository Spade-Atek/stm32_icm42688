...
/*******************************************************************************
* Ãû    ³Æ£º bsp_IcmGetRawData
* ¹¦    ÄÜ£º ¶ÁÈ¡Icm42688¼ÓËÙ¶ÈÍÓÂÝÒÇÊý¾Ý
* Èë¿Ú²ÎÊý£º ÁùÖá
* ³ö¿Ú²ÎÊý£º ÎÞ
* ×÷¡¡¡¡Õß£º Baxiange
* ´´½¨ÈÕÆÚ£º 2024-07-25
...
int8_t bsp_IcmGetRawData(icm42688RealData_t* accData, icm42688RealData_t* GyroData)
{
	...
	inv_icm426xx_read_reg(&icm_driver, MPUREG_ACCEL_DATA_X0_UI, 12, buffer);
    ...
    accData->x = (float)(accRaw.x * 8000 / 32768.0f); // 8G
    accData->y = (float)(accRaw.y * 8000 / 32768.0f);
    accData->z = (float)(accRaw.z * 8000 / 32768.0f);

    GyroData->x = (float)(gyroRaw.x * 1000.0f / 32768.0f);  // 1000dps
    GyroData->y = (float)(gyroRaw.y * 1000.0f / 32768.0f);
    GyroData->z = (float)(gyroRaw.z * 1000.0f / 32768.0f);
