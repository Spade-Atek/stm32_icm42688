...
//#define SPI_IMU_CS=0     HAL_GPIO_WritePin(ICM_PORT_SPIX_CS, ICM_PIN_SPIX_CS, GPIO_PIN_RESET)
//#define SPI_IMU_CS=1    HAL_GPIO_WritePin(ICM_PORT_SPIX_CS, ICM_PIN_SPIX_CS, GPIO_PIN_SET)
...
static uint8_t icm42688_read_reg(uint8_t reg)
{
    ...
    /* 读取寄存器数据 */
    regval = SPI2_ReadWriteByte(0xFF);
    SPI_IMU_CS=1;
#elif defined(ICM_USE_I2C)
